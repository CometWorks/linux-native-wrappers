...
renames = dict(line.rstrip("\n").split("\t", 1)
               for line in open(manifest_path, encoding="utf-8") if line.strip())
if len(renames) != 219 or any(len(old) != len(new) for old, new in renames.items()):
    raise SystemExit(f"expected 219 equal-length Physics export mappings, found {len(renames)}")
...
for index in range(1, symbol_count):
    ...
    old = data[name_start:name_end].decode(errors="ignore")
    if old not in renames:
        continue
    data[name_start:name_end] = renames[old].encode()
    renamed += 1
