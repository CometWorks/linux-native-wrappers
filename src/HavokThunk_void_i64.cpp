...
static std::atomic_uintptr_t void_i64_target = 0;
static uint32_t void_i64_refcount = 0;  // guarded by void_i64_mutex

static void WINAPI void_i64_thunk(int64_t arg0)
{
    auto fn = reinterpret_cast<void_i64_sysv_t>(void_i64_target.load(std::memory_order_acquire));
    if (!fn) {
        return;
    }
    fn(arg0);
...
void *bridge_void_i64(void *target_ptr)
{
    ...
    auto current = void_i64_target.load(std::memory_order_acquire);
    if (current == 0) {
        void_i64_target.store(reinterpret_cast<uintptr_t>(target_ptr), std::memory_order_release);
        void_i64_refcount = 1;
    } else if (current == reinterpret_cast<uintptr_t>(target_ptr)) {
        ++void_i64_refcount;
    } else {
        fprintf(stderr,
                "FATAL: Havok callback bridge single-slot family 'void_i64' received a\n"
    ...
    return reinterpret_cast<void *>(&void_i64_thunk);
}
...
void release_void_i64(void *target_ptr)
{
    ...
    std::lock_guard<std::mutex> lock(void_i64_mutex);
    if (void_i64_target.load(std::memory_order_acquire) != reinterpret_cast<uintptr_t>(target_ptr)) {
        return;
    }
    if (void_i64_refcount > 0 && --void_i64_refcount == 0) {
        void_i64_target.store(0, std::memory_order_release);
    }
}
