...
static std::atomic_uint32_t void_i64_refcounts[256] = {};
...
void *bridge_void_i64(void *target_ptr)
{
    ...
    auto target = reinterpret_cast<void_i64_sysv_t>(target_ptr);
    std::lock_guard<std::mutex> lock(void_i64_mutex);
    for (size_t i = 0; i < 256; ++i) {
        if (reinterpret_cast<void_i64_sysv_t>(void_i64_targets[i].load(std::memory_order_acquire)) == target) {
            void_i64_refcounts[i].fetch_add(1, std::memory_order_relaxed);
            return reinterpret_cast<void *>(void_i64_thunks[i]);
        }
    }
    for (size_t i = 0; i < 256; ++i) {
        if (void_i64_targets[i].load(std::memory_order_acquire) == 0) {
            void_i64_targets[i].store(reinterpret_cast<uintptr_t>(target), std::memory_order_release);
            void_i64_refcounts[i].store(1, std::memory_order_relaxed);
            return reinterpret_cast<void *>(void_i64_thunks[i]);
        }
    }
    fprintf(stderr,
...
void release_void_i64(void *target_ptr)
{
    ...
    for (size_t i = 0; i < 256; ++i) {
        if (reinterpret_cast<void_i64_sysv_t>(void_i64_targets[i].load(std::memory_order_acquire)) == target) {
            auto refs = void_i64_refcounts[i].load(std::memory_order_relaxed);
            if (refs > 0) {
                refs = void_i64_refcounts[i].fetch_sub(1, std::memory_order_acq_rel) - 1;
            }
            if (refs == 0) {
                void_i64_targets[i].store(0, std::memory_order_release);
            }
            return;
        }
    }
