...
static uint32_t void_ptr_high_water = 0;
...
// list (&void_ptr_thunk<0> .. &void_ptr_thunk<127>), one instantiation per slot.
#define HKTHUNK_1(n) &void_ptr_thunk<(n)>
...
static void_ptr_ms_t void_ptr_thunks[128] = { HKTHUNK_128(0) };
#undef HKTHUNK_1
#undef HKTHUNK_2
#undef HKTHUNK_4
#undef HKTHUNK_8
...
void *bridge_void_ptr(void *target_ptr)
{
    ...
    auto existing = void_ptr_index.find(target_ptr);
    if (existing != void_ptr_index.end()) {
        ++existing->second.refs;
        return reinterpret_cast<void *>(void_ptr_thunks[existing->second.index]);
    }
    uint32_t index;
    ...
    void_ptr_targets[index].store(reinterpret_cast<uintptr_t>(target_ptr), std::memory_order_release);
    void_ptr_index.emplace(target_ptr, void_ptr_slot{index, 1});
    return reinterpret_cast<void *>(void_ptr_thunks[index]);
}
...
void release_void_ptr(void *target_ptr)
{
    ...
    std::lock_guard<std::mutex> lock(void_ptr_mutex);
    auto it = void_ptr_index.find(target_ptr);
    if (it == void_ptr_index.end()) {
        return;
    }
    if (--it->second.refs == 0) {
        uint32_t index = it->second.index;
        void_ptr_targets[index].store(0, std::memory_order_release);
        void_ptr_free_slots.push_back(index);
        void_ptr_index.erase(it);
    }
}
