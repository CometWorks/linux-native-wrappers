...
- **Per-instance delegates** — `HkPhantomCallbackShape` is the only wrapper that
  ...
  2 `void_ptr_ptr` slots (enter/leave) + 1 `void_ptr` slot (delete). The game
  creates one phantom shape per trigger/detector volume — ship connectors and
  ejectors, collectors, gravity generators, merge blocks, safe zones, and opt-in
  detector entities — so **`void_ptr` and `void_ptr_ptr` scale with block count**
  and get **16384** slots. Loading *"Many Lifters Slowness"* (699 grids)
  exhausted the previous flat limit of 4096 for `void_ptr_ptr` and aborted.
...
This keeps `libHavok.so` at ~22 MB (versus ~87 MB if every family used 16384).
